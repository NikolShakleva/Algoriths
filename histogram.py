...
dir_file = dir + '2020.10.31-18.03.29/'
...
def prepare(df):
    ...
    std2 = [mu + (2.0 * std), mu - (2.0 * std)]     # standard diviation σ2

    # dropping data not in σ1 or σ2
    index = df[(df['Output'] > std2[0])|(df['Output'] < std2[1])].index
    df.drop(index, inplace = True)
    
    div = n/10.0                    # finding number to divide data with
...
def make_hist(d, mu, std1, std2, m, n):
    ...
    # Plot the histogram.
    plt.hist(d, bins=20, density=True, alpha=0.6, color='cornflowerblue')
    y_pos = 0.025
    ...
    plt.axvline(std2[0], color='dimgrey', linestyle='dashed', linewidth=0.5)
    plt.axvline(std2[1], color='dimgrey', linestyle='dashed', linewidth=0.5)
    plt.axvline(mu,      color='dimgrey', linestyle='dashed', linewidth=0.5)
    ...
    plt.title(title)
    plt.savefig( dir +'/Fig/HyperLogLog_M' + str(m)+ '_N' + str(n))
    plt.show()
